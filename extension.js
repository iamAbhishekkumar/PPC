...
function activate(context) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("ppc.createPythonProject", async () => {
      ...
      if (currentDirectory != null) {
        try {
          ...
          quickPick.onDidChangeSelection(async ([{ label }]) => {
            ...
            if (folderName != null && folderName.length != 0) {
              switch (label) {
                ...
                case "Flask-App: Advanced": {
                  createAdvancedFlaskAppStructure(
                    currentDirectory + "/" + folderName,
                    folderName
                  );
                  break;
            ...
            } else vscode.window.showErrorMessage("Enter a valid project name");
            quickPick.hide();
          });
          quickPick.show();
...
function createBasicFlaskAppStructure(folderPath, projectName) {
  if (!fs.existsSync(folderPath)) {
    ...
    gen.genFiles(
      folderPath,
      "config.py",
      "# All your permanent configurations goes here."
    );
    gen.genGitIgnore(folderPath);
    gen.genLicense(folderPath);
    gen.genReadMe(folderPath, projectName);
...
function createAdvancedFlaskAppStructure(folderPath, projectName) {
  if (!fs.existsSync(folderPath)) {
    ...
    gen.genFiles(
      `${folderPath}/app/auth`,
      "forms.py",
      "# Create your http forms here"
    );
    gen.genFiles(
      `${folderPath}/app/auth`,
      "commands.py",
      "# Custom terminal commands to ease your workflow. "
    );
    ...
    gen.genFiles(
      folderPath,
      "wsgi.py",
      "# wsgi module to integrate app with web servers like apache, gunicorn. "
    );
    gen.genEnvironment(folderPath);
  } else {
    alreadyExistsMessage(folderPath);
...
function alreadyExistsMessage(folderPath) {
  vscode.window.showErrorMessage(
    `Already exists a folder named as ${
